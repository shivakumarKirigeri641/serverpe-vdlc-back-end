...
// Brand logo — loaded once at module load; falls back to text-only if missing.
...
const POWERED_BY      = "Powered by: ServerPe App Solutions";
...
const generateReportPdf = (p) => {
  try {
    ...
    doc.setFontSize(16);
    doc.text(sanitize(brand || PLATFORM_NAME), textX, 40);
    doc.setFont("helvetica", "normal");
    doc.setFontSize(8);
    doc.text(sanitize(PLATFORM_NAME), textX, 54);
    doc.text(sanitize(PLATFORM_DOMAIN), textX, 66);

    // Right: vehicle + report type + date
    doc.setFont("helvetica", "bold");
    doc.setFontSize(14);
    ...
    doc.text(`Report generated by ${PLATFORM_NAME} | ${PLATFORM_DOMAIN}`, M, footerY + 11);

    // Right: powered by
    doc.setFont("helvetica", "bold");
    doc.setFontSize(7.5);
    doc.setTextColor(...GOLD);
    doc.text(sanitize(POWERED_BY), W - M, footerY + 11, { align: "right" });
    ...
    const disclaimer =
      ...
      "ServerPe App Solutions is not liable for any errors or decisions made solely on the basis of this report. " +
    ...
    // Brand watermark on every page + sample overlay if requested.
    ...
    for (let i = 1; i <= pages; i++) {
      ...
      doc.setFontSize(36);
      doc.setTextColor(...BLUE);
      doc.text(sanitize(PLATFORM_NAME), W / 2, H / 2 - 18, { align: "center", baseline: "middle", angle: -30 });
      doc.setFontSize(16);
      doc.text(sanitize(PLATFORM_DOMAIN), W / 2, H / 2 + 22, { align: "center", baseline: "middle", angle: -30 });
      if (typeof doc.GState === "function") doc.setGState(new doc.GState({ opacity: 1 }));
      // Sample overlay (bolder, on top of brand watermark)
      if (p.watermark) {
        if (typeof doc.GState === "function") doc.setGState(new doc.GState({ opacity: 0.10 }));
        doc.setFont("helvetica", "bold");
        doc.setFontSize(70);
        doc.setTextColor(...BLUE);
